...
function Grader_Tasks() {
    ...
    const scores = Array.from(document.querySelectorAll('.line'))
        .filter((line) => isNaN(line.querySelector('.label-score')))
        .reverse();
    scores.forEach((element) => {
        let id = element.querySelector('a').href.split('/');
        element.task_id = id[id.length - 1];
    });
    if (scores.length === 0) {
        return;
    }
    ...
    if (!isNaN(total_score / total_potential)) {
        ...
        dataScoreSort
            .filter((part) => part.gained < part.total)
            .forEach((pointlost) => {
            assignment_list_string += `${pointlost.name} (Was Due on: ${pointlost.date}, Affects Grade: ${
                (total_score / total_potential - (pointlost.total - pointlost.gained + total_score) / total_potential) * 100
            }%)<br/>`;
            });
